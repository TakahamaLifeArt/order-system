<?php
	...
	// インクジェット担当
	$staff_selector = '<select name="state_6" id="state_6" class="staff" rel="rowid6">';
	try{
		$conn = db_connect();
...
<head>
	...
	<script type="text/javascript" src="./js/inkjetlist.js"></script>
							...
							<table>
								<tbody>
									<tr>
										<th>インクジェット</th>
										<td>
											<select name="fin_6" id="fin_6">
											<?php
												if(!isset($q['fin_6'])) $q['fin_6'] = 1;
												$tmp = '<option value="0">全て</option><option value="1">未終了</option><option value="2">終了</option>';
												echo preg_replace('/value=\"'.$q['fin_6'].'\"/','value="'.$q['fin_6'].'" selected="selected"',$tmp);
											?>
											</select>
										</td>
