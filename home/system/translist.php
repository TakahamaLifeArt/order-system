<?php
	...
	// 転写紙担当
	$staff_selector = '<select name="state_3" id="state_3" class="staff" rel="rowid4">';
	try{
		$conn = db_connect();
		$result = exe_sql($conn, 'SELECT * FROM staff where rowid4>0 and staffapply<=curdate() and staffdate>adddate(curdate(), interval -30 day) order by rowid4 ASC');
...
<head>
	...
	<script type="text/javascript" src="./js/translist.js"></script>
			...
			<div>
				<fieldset>
							...
							<table>
								<tbody>
									<tr>
										<th>転写紙</th>
										<td>
											<select name="fin_3" id="fin_3">
											<?php
												if(!isset($q['fin_3'])) $q['fin_3'] = 1;
												$tmp = '<option value="0">全て</option>
														<option value="1">未終了</option>
														<option value="2">作成済</option>';
												echo preg_replace('/value=\"'.$q['fin_3'].'\"/','value="'.$q['fin_3'].'" selected="selected"',$tmp);
											?>
											</select>
										</td>
					...
					<p class="btn_area">
						...
						<input type="button" value="シート数-年度集計" id="addup">
