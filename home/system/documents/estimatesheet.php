...
*/
	...
	if(isset($_REQUEST['orderid'],$_REQUEST['param'])){
		...
		$orders_id = 	 htmlspecialchars(mb_convert_encoding($_REQUEST['orderid'],'utf-8','euc-jp'), ENT_QUOTES);
		...
		*/
		...
		$output_date = date(Y).'-'.date(m).'-'.date(d);
		
		// 出力フォーム
		$html = '
		<div class="heading1" style="margin:-30 auto 20;letter-spacing:2mm;font-size:12pt;">御　見　積　書<span class="copy"></span></div>
		<div class="toright" style="margin:0;">見積日 '.$output_date.'　　見積No. '.sprintf('%09d',$orders_id).'</div>
	    ...
	    $deli = explode('-',$orders['schedule4']);					// 受渡日付の配列
		switch($orders['payment']){									// 取引方法
		...
		$html .= '
		<p style="margin:10 0 0 0;">下記の通り御見積り申し上げます。ご検討の程宜しくお願い申し上げます。</p>
		<p style="font-size:14pt;font-weight:bold;margin:0;">御見積り金額　<ins style="font-size:100%;font-weight:bold;color:#003f75;">&yen;'.number_format($sum).' －</ins> （消費税含む）</p>
		<table>
			<tbody>
				<tr><td>受渡期日</td><td>'.$deli[0].'年'.$deli[1].'月'.$deli[2].'日</td></tr>
				<tr><td>取引方法</td><td>'.$payment.'</td></tr>
				<tr><td>有効期限</td><td>'.$expire.'</td></tr>
			</tbody>
		</table>';
