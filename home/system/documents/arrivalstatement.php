 ...
 * 
 */
if(!isset($_GET['orderid'])) exit('No such file exists');
$root_path = "../";
...
$DB = new Orders();
//$orders_id = htmlspecialchars($_GET['orderid'], ENT_QUOTES);
//$print_key = htmlspecialchars($_GET['printkey'], ENT_QUOTES);

// 受注書データを取得
$datas = $DB->db( 'search','arrivalstatement',array('id'=>implode(',', $_GET['orderid'])) );
if(empty($datas)) exit('No such top data exists');
...
<?php
for($q=0; $q<count($datas); $q++){
	$orders = $datas[$q];
	$orders_id = $orders['orders_id'];
	$print_key = $orders['printtype_key'];
	$print_name = '';
		...
		<div class="wrap" {$extheight}>
			...
			<div class="col1">
				<div id="heading" class="clearfix">
					...
					<div id="customer_wrap" {$skin}>
						<table>
							<tbody>
								<tr>
									<th rowspan="2" style="height:106px;">
										<p>顧客ID</p>
