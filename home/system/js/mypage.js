...
*/
var mypage = {
	prop: {'modified':false,			// 修正フラグ　true: 修正あり
	...
	order_info: {
		id: ['order_id','reception','destination','order_comment','paymentdate','exchink_count','deliverytime','manuscriptdate','invoicenote','billnote',
	...
	init: function(){
		$.ajax({url:'./php_libs/set_tablelist.php', type:'POST', dataType:'text',data:{'act':'staff', 'rowid':1, 'curdate':mypage.prop.created}, async:false,
			success:function(r){
				$('#reception').html(r);
				$('#log_staff').html(r);
			}
		...
		$.ajax({url:'./php_libs/set_tablelist.php', type:'POST', dataType:'text', async:false, data:{'act':'category', 'ordertype':mypage.prop.ordertype, 'curdate':mypage.prop.firmorderdate}, success:function(r){$('#categoryIs').html(r);} });
		$.ajax({url:'./php_libs/set_tablelist.php', type:'POST', datatype:'text', async:false, data:{'act':'item','current_id':1, 'curdate':mypage.prop.firmorderdate}, 
			success:function(r){
	...
	*	消費税率を設定
	*/
		$.ajax({url:'./php_libs/dbinfo.php', type:'POST', dataType:'text', data:{'act':'tax', 'curdate':args, 'ordertype':mypage.prop.ordertype}, async:false,
			success:function(r){
				if(r.match(/^\d+?$/)){
					r/=100;
				}else{
	...
	hide_uploader: function(){
		...
		currImg.attr({'src':newsrc, 'alt':orgname, 'width':w}).show();
		pos.val("");
		wrapper.children('.freeimage').hide();
	},
	...
	*/
		...
		$.ajax({url:'./php_libs/ordersinfo.php', type:'POST', dataType:'text', async:true, 
			data:{'act':'search', 'mode':'numberOfBox', 'field1[]':field1, 'data1[]':data1, 'field4[]':field4, 'data4[]':data4}, success:function(r){
				if(r.match(/^\d+?$/)){
					$('#boxnumber').val(r);
				}else{
					alert('Error: p634'+r);
	...
	*/
		var size_hash = {'70':1,'80':2,'90':3,'100':4,'110':5,'120':6,'130':7,'140':8,'150':9,'160':10,
	...
	*	size_name:	ソート用
	*/
		name: ['maker','master_id','item_name','color_code','size_id','size_name','amount','cost','choice','stock_number']
	},
	...
	*/
	...
	/*
	*	sessionStorageに格納
	*	@list	[{'maker', 'master_id','item_name','color_code','size_id','size_name','amount','cost','choice','stock_number'},{},{}]
	*			default
	*				cost:0
	*				choice:1
	*				stock_number = ''
	...
	*/
		...
		/*
		*	sessionStorageに保存
		*	[{'maker','master_id','item_name','color_code','size_id','size_name','amount','cost','choice','stock_number'},{}.{}]
		*/
		var args = [];
		...
		$.ajax({url:'./php_libs/dbinfo.php', type:'POST', dataType:'json', async:false, 
			data:list, 
			success:function(r){
					...
					if(!isExistitem){
						...
						if($('#pp_toggler_'+category_id).length==0){
							...
							$.ajax({url:'./php_libs/dbinfo.php', type:'POST', dataType:'text', async:false,
								data:{'act':'printposition','item_id':item_id,'curdate':mypage.prop.firmorderdate, 'ordertype':mypage.prop.ordertype},
								success:function(data){
									...
									html += '</div>';
									html +=	'<p class="title">'+category_name+'<span title="item_'+item_id+'">'+item_name+dry+'</span></p>';
									html += '</div>';
							...
							$('#pp_toggler_'+category_id+' p').append('<span title="item_'+item_id+'">'+item_name+dry+'</span>');
						}
						$('#pp_wrapper').find('.repeat_check').change(function(){mypage.calcPrintFee();});
						*/
					}
					mypage.calcPrintFee();
				}else{
	...
	***********************************************/
    ...
    $("#deleteImg").die("click");
		...
		$.ajax({url:'./php_libs/design.php', type:'POST', dataType:'json', async:false,
			data:{'act':'showDesignImg', 'order_id':orders_id, 'folder':'attatchfile'},
			success:function(data){
				if(data==""){
	...
	//					window.clearInterval(mypage.prop.intervalID);
		...
		$('#deleteImg').live('click', function(){
			...
			$.ajax({url: './php_libs/design.php', type:'POST', dataType:'text', async:false,
				data:{'act':'deleteDesFile', 'order_id':orders_id,  'file_name':file_name, 'folder':'attatchfile'}, 
				success: function(r){
					if(r==1){
	...
	uploadDesignImg: function(orders_id_i){
   	...
   	$("#desImgcancel").die("click");
		...
		$('#desImgup').live('click', function(){
			...
			orders_id= $('#uploadImg_table').find("#order_id").val();
			$.ajax({url: './php_libs/design.php', type:'POST', dataType:'text', async:false,
				data:{'act':'checkFileName', 'order_id':orders_id, 'file_name':file_name, 'folder':'attatchfile'}, 
				success: function(r){
					if(r){
	...
	***********************************************/
	...
	//					window.clearInterval(mypage.prop.intervalID);
		...
		$('#deleteDesedImg').live('click', function(){
			...
			$.ajax({url: './php_libs/design.php', type:'POST', dataType:'text', async:false,
				data:{'act':'deleteDesFile', 'order_id':orders_id, 'file_name':file_name, 'folder':'imgfile'}, 
				success: function(r){
					if(r==1){
    ...
    $("#desedImgcancel").die("click");
		...
		$('#desedImgup').live('click', function(){
			...
			orders_id= $('#uploadDesedImg_table').find("#order_id").val();
			$.ajax({url: './php_libs/design.php', type:'POST', dataType:'text', async:false,
				data:{'act':'checkFileName','order_id':orders_id, 'file_name':file_name, 'folder':'imgfile'}, 
				success: function(r){
					if(r){
						$.msgbox('同名ファイルは存在しています');
						return;
	...
	*/
		...
		*/
		...
		mypage.showOrderItem({'orders_id':orders_id, 'noprint':noprint});
	...
	*/
		...
		if(id==0){
			...
			$.ajax({url:'./php_libs/dbinfo.php', type:'POST', dataType:'text', async:false, data:{'act':'size','item_id':id, 'ordertype':mypage.prop.ordertype, 'curdate':mypage.prop.firmorderdate}, 
				success:function(r){ $('#size_table').html(r); }
			});
			mypage.setItemInfo(',0,,0,0,');
		}else{
			$.ajax({url:'./php_libs/dbinfo.php', type:'POST', dataType:'text', async:false, data:{'act':'itemimage','item_id':id,'color_code':code, 'curdate':mypage.prop.firmorderdate}, 
				success:function(r){
					var info = mypage.setItemInfo(r);
					var isPrint = $('#noprint').is(':checked')? 0: 1;
					if($('#noprint').is(':checked')) isPrint = 0;
					$.ajax({url:'./php_libs/dbinfo.php', type:'POST', dataType:'text', async:false, data:{'act':'size','item_id':id, 'itemcolor_code':info[1], 'isprint':isPrint, 'ordertype':mypage.prop.ordertype, 'curdate':mypage.prop.firmorderdate}, 
						success:function(r){
							$('#size_table').html(r);
						}
					});
	...
	*/
		...
		// sessionStorageの削除
		var args = {'master_id':master_id, 'size_id':size_id};
	...
	*/
		...
		$.ajax({url:'./php_libs/dbinfo.php', type:'POST', dataType:'json', async:false,
			data:{'act':'changeitem', 'item_id':new_item_id, 'size_id':size_id, 'color_name':color_name, 'curdate':mypage.prop.firmorderdate}, 
			success:function(r){
				if(r instanceof Array){
					if(r[0]==''){
						$(my).val(pre_item_id);
					...
					$('#orderlist tbody tr').each( function(index){
						...
						if(cate==category_id && ppID==pre_ppID) flg_ppid = true;
					});
							...
							if(r instanceof Array){
								...
								if($('#pp_wrapper span[title="item_'+new_item_id+'"]').length==0){
									if( $("#pp_toggler_"+category_id).next().find('.printposition_'+new_ppID).length==0 ){
										$.ajax({url:'./php_libs/dbinfo.php', type:'POST', dataType:'text', async:false, data:{'act':'printposition','item_id':new_item_id, 'curdate':mypage.prop.firmorderdate, 'ordertype':mypage.prop.ordertype}, 
											success:function(data){
												$('#pp_toggler_'+category_id).next().append(data);
												$('#pp_wrapper :input').change(function(){mypage.prop.modified = true;});
											}
								...
								if(!flg_id){
									...
									if(toggle.children('span').length==1){
										...
										if(!flg_ppid) toggle.parent().next().find('.printposition_'+pre_ppID).remove();
									}
								}
	...
	*/
		...
		case 'categoryIs':
			...
			if(current_id==0 || current_id>=99){
				$.ajax({url:'./php_libs/dbinfo.php', type:'POST', dataType:'text', async:false, data:{'act':'size','item_id':current_id, 'ordertype':mypage.prop.ordertype, 'curdate':mypage.prop.firmorderdate}, 
					success:function(r){ $('#size_table').html(r); }
				});
				...
				$('#stock_number, #maker').show();
				$('#itemcolor_name, #stock_number, #maker').attr('readonly','readonly').addClass('readonly');
				$.ajax({url:'./php_libs/set_tablelist.php', type:'POST', dataType:'text', async:false, data:{'act':'item','current_id':current_id, 'curdate':mypage.prop.firmorderdate}, 
					success:function(r){
						$('#itemIs').html('<select id="item_selector" onchange="mypage.changeValue(this)">'+r+'</select>');
	...
	showInkcolor: function(my){
		...
		var palettename = arguments.length==1? 'inkcolor': 'cuttingcolor';
		$('#inkcolor_list').load('./txt/'+palettename+'_palette.txt',
			function(){
				...
				$('#inkcolor_wrapper').css({'top':offsetY+'px'}).fadeIn();
			}
		);
	},
	changeInkcolor:function(my){
		...
		if(code=='c00'){
			self.attr({'src':'./img/undefined.gif', 'alt':code});
			self.next().removeAttr('readonly').val('');
		}else{
			self.attr({'src':'./img/inkcolor/'+code+'.png', 'alt':code});
	...
	changeCuttingcolor:function(my){
		...
		if(code=='000'){
			...
			self.attr({'src':'./img/cuttingcolor/'+code+'.png', 'alt':code});
			self.next().attr('readonly', true).val(inkname);
		}
	...
	*/
		...
		// Self-Design
			...
			var inkjet_palecolor_code = {"001":true};	// 淡色
			var tot_price = 0;
			...
			$('#orderlist tbody tr').each( function(){
				...
				if(typeof items[item_id] == 'undefined'){
					...
					items[item_id][color_code] = {'category_id':categoryid, 'vol':vol, 'ppID':ppID, 'printfee':0};
		...
		// 通常
			...
			$('#orderlist tbody tr').each( function(){
				...
				if(categoryid==0 || categoryid==100){ // その他と持込
					...
					if(typeof ca[ratioID] == 'undefined'){
						...
						ca[ratioID][plates][categoryid][ppID] = {'item_id':[], 'vol':vol};
				...
				if(typeof itemprintfee[item_id] == 'undefined'){
					itemprintfee[item_id] = {'vol':vol, 'fee':0, 'name':itemname, 'cost':cost};
				}else{
			...
			var plate_check = {'silk':[],'inkjet':[],'darkinkjet':[],'cutting':[],'digit':[],'trans':[],'darktrans':[]};
			plate_check['silk'] = [{},{},{},{},{}];
			plate_check['inkjet'] = [{},{},{},{},{}];
			...
			if(plate_type.length>0){
				...
				var postData = {'act':'printfee', 'pos':print_pos, 'name':print_name, 'area':print_area, 'ink':ink_count, 'size':print_size, 'plates':plate_type,
					'amount':print_amount, 'ratio':print_ratio, 'extra':extra_ratio, 'item_id':itemid, 'repeat':repeat_type, 'setting':setting_group, 
					'curdate':mypage.prop.acceptingdate
				};
				$.ajax({url: './php_libs/estimation.php', type:'POST', dataType:'json', data:postData, async:false,
					success: function(r){
						if(r instanceof Array){
			...
			if(repeat_all_check && mypage.prop.repeat>0 && mypage.prop.reuse==1 && !$('#free_printfee').is(':checked')){
				...
				var target_item = {};
				// 版元に無いアイテムの有無を確認
				$.ajax({url:'./php_libs/ordersinfo.php', async:false, dataType:'json', data:{'act':'search','mode':'orderitemlist', 'field1[]':['orders_id'], 'data1[]':[mypage.prop.repeat]}, 
					success: function(r){
						if(r instanceof Array){
							var base_item = {};
				...
				// 版元に無いプリントの有無を確認
				if(mypage.prop.isRepeatFirst==true){
					$.ajax({url:'./php_libs/ordersinfo.php', async:false, dataType:'json', data:{'act':'search','mode':'orderprint', 'field1[]':['orders_id'], 'data1[]':[mypage.prop.repeat]}, 
						success: function(r){
							if(r instanceof Array){
								...
								// インク情報
								var base_ink = [];
								$.ajax({url:'./php_libs/ordersinfo.php', async:false, dataType:'json', data:{'act':'search','mode':'orderink', 'field1[]':['orders_id'], 'data1[]':[mypage.prop.repeat]}, 
									success: function(r){
										for(var i=0; i<r.length; i++){
	 ...
	 */
		...
		// スケジュールが未定の場合
			...
			*/
			...
			$.ajax({url: './php_libs/deliveryDate.php', type: 'POST',dataType: 'text',async: false,
				data:{'act':'works','base':base,'send':send,'deli':deli},
				success:function(r){
					var workday = r.split(',');
		...
		if(mypage.prop.reuse==1 && !$('#free_printfee').is(':checked') && mypage.prop.isRepeatFirst && discount_ratio==0){
			var baseinfo = '';
			var baseprintfee = {};
			var postData = {'act':'baseprintfee', 'orders_id':mypage.prop.repeat};
			$.ajax({url: './php_libs/estimation.php', type:'POST', dataType:'json', data:postData, async:false,
				success: function(r){
					if(r instanceof Array){
						baseinfo = r[0];
	...
	*/
		...
		if(print_type=='silk'){
			...
			my.closest('.pp_info').siblings('.pp_ink').children('p').children('.pos_name').each( function(){
				$(this).next('img').attr({'src':'./img/circle.png'}).next('input').val('');
			});
		...
		switch(print_type){
			...
			case 'inkjet':
				html = '版 <select class="areasize_id" onchange="mypage.limit_size(this)">' +
					   '<option value="0" selected="selected">大（40×28）</option>' +
					   '<option value="1">中（28×20）</option>' +
					   '<option value="2">小（15×15）</option>' +
					   '</select>';
				html2 = 'サイズ <input type="text" value="" class="design_size" />';
			...
			case 'trans':
				html = '版 <select class="areasize_id" onchange="mypage.limit_size(this)">' +
					   '<option value="0" selected="selected">大（38×27）</option>' +
			...
			case 'cutting':
				html = '版 <select class="areasize_id" onchange="mypage.limit_size(this)">' +
					   '<option value="0" selected="selected">大（40×30）</option>' +
					   '<option value="1">中（30×10）</option>' +
					   '<option value="2">小（10×10）</option>' +
					   '</select>';
				html2 = 'サイズ <input type="text" value="" class="design_size" />';
				html3 = '';
	...
	*/
		var print_type = '';
		if(arguments.length==2){
			print_type = arguments[1];
			var num = my.value-0;
		    var opp = $(my).siblings('input').val() - 0;

			switch(print_type){
				case 'silk':
					num = mypage.check_NaN(my);
					if(opp<=27){
						if(num>35) num=35;
					}else{
						if(num>27) num=27;
		...
		if(print_type != 'silk') mypage.calcPrintFee();
		mypage.prop.modified = true;
	},
	...
	setAddr: function(num,zipcode,addr0,addr1,addr2){
		...
		$('#address_wrapper'+num).fadeOut('normal', function(){$('#address_list'+num).html('');});
	},
	save: function(mode){
	/*
	 *	保存
	 ...
	 */
		...
		case 'firstcontact':
			...
			$.ajax({url:'./php_libs/ordersinfo.php', type:'POST', async:false,
				data:{'act':'insert','mode':'firstcontact', 'field1[]':field1, 'data1[]':data1}, success:function(r){
					if(r.trim()==''){
						alert('Error: p1692\n'+r);
						isReturn = false;
						return;
		...
		case 'order':
			...
			if(mypage.prop.ordertype=="general" && mypage.prop.repeat>0){
				$.ajax({url:'./php_libs/ordersinfo.php', type:'POST', dataType:'json', async:false,
					data:{'act':'search','mode':'reuse', 'field1[]':['id'], 'data1[]':[mypage.prop.repeat]}, success: function(r){
						if(r instanceof Array){
							var len = r.length;
							// 確定注文の場合は当該注文も含む
							if(mypage.prop.firmorder){
			...
			if(mypage.prop.ordertype=="general"){
				...
				$('#est_table1 tbody th:not(.sub)').each( function(){ valid1+=$(this).next().text().replace(/,/g, '')-0; });
		...
		//------------------------
			...
			if(mypage.prop.ordertype=="general"){
				...
				//------------------------
				var ary = ['productfee','printfee','silkprintfee','colorprintfee','digitprintfee','inkjetprintfee','cuttingprintfee',
				...
				$('#itemprint tbody tr').each( function(){
					...
					itemprice_hash[itemid] = {'fee':$(this).find('.fee').text().replace(/,/g,''), 'one':$(this).find('.perone').text().replace(/,/g,'')};
				});
...
//return;
		...
		//------------------------
			$.ajax({url:'./php_libs/ordersinfo.php', type:'POST', async:false,
				data:{'act':action,'mode':'order', 'field1[]':field1, 'data1[]':data1, 'field2[]':field2, 'data2[]':data2,
				...
				'field10[]':field10, 'data10[]':exchink, 'field12[]':field12, 'data12[]':data12}, success:function(r){
					if(r.trim()==''){
						alert('Error: p2875\n'+r);
						isReturn = false;
						return;
					...
					if($('#order_completed:visible').length>0){
						$.ajax({url:'./php_libs/ordersinfo.php', type:'POST', dataType:'text',
							data:{'act':'insert', 'mode':'direction', 'field1[]':['orders_id','ordertype'],
							'data1[]':[orders_id, mypage.prop.ordertype]}, async:false,
							success: function(r){}
						});
					}
				}
		...
		// 当該プリントタイプについてのみ登録
			...
			$.ajax({url: './php_libs/ordersinfo.php', type: 'POST', async: false,
				data: {'act':'update','mode':'direction','field1[]':field,'data1[]':data,
				'field4[]':field4,'data4[]':data4, 'field5[]':field5,'data5[]':data5, 'field6[]':field6,'data6[]':data6, 
				'field7[]':field7,'data7[]':data7, 'field8[]':field8,'data8[]':data8},
 				success: function(r){
					if(r.match(/^\d+?$/)){
						product_id=r;
					}else{
	...
	setBundle: function(){
		...
		$.ajax({url: './php_libs/ordersinfo.php', type: 'POST', async: false,
			data: {'act':'update','mode':'bundle','field5[]':field5,'data5[]':data5},
			success: function(r){
				if(r.match(/^\d+?$/)){
					if(r>1 && isMine){
						$('#bundle_status').show();
	...
	confirm:function(){
		...
		if( $('#reception').val()=="" || schedule2=="" || schedule4==""){
			$('#alert_require').effect('pulsate',{'times':4},250);
	...
	*/
		...
		var factory_hash = {1:'１工場', 2:'２工場', 9:'混合'};
		...
		if(arguments.length>1){				// セレクターでプリントタイプの変更
			...
			$.ajax({
				url: './php_libs/ordersinfo.php', type: 'POST', dataType: 'text',
				data: {'act':'search', 'mode':'printinfo', 'field1[]':['orders_id','print_key'], 'data1[]':[orders_id,print_type]}, async: false,
				success: function(r){
					Err = true;
					if(r.trim()==""){
						return;
			 ...
			 */
			$.ajax({url:'./php_libs/ordersinfo.php', type:'POST', dataType:'text',
				data:{'act':'insert', 'mode':'direction', 'field1[]':['orders_id','ordertype','printinghash'],
				'data1[]':[orders_id, mypage.prop.ordertype, 1]}, async:false,
				success: function(r){
					var data = r.split('|');
					var l = data.length;
					if(l==0 || data[0]==""){
			...
			$.ajax({
				url: './php_libs/ordersinfo.php', type: 'POST', dataType: 'text',
				data: {'act':'search', 'mode':'product', 'field1[]':['orders_id','order_type'],
				'data1[]':[orders_id,mypage.prop.ordertype]}, async: false,
				success: function(r){
					Err = true;
					if(r.trim()==""){
					...
					if(lines.length>0){
						for(var i=0; i<lines.length; i++){
							...
							if(res['area_name']=="free"){
								...
								$.ajax({
									url: res['area_path'], type: 'POST', dataType: 'text', async: false,
									success: function(r){
										if(r.trim()==""){
											alert('Error: p2893\n'+r);
											Err = true;
		...
		if(print_type=='digit'){
			...
			$.ajax({
				...
				data: {'act':'search', 'mode':'cutpattern', 'field1[]':['product_id'],'data1[]':[directions[0]['product_id']]}, 
				success: function(r){
					if(r instanceof Array){
						if(r.length>0){
							var cutpattern = '<tr>';
		...
		switch(directions[0]['carriage']){
			case 'normal':	$('#shipment').text('宅急便');
							break;
			case 'accept':	$('#shipment').text('工場渡し');
							break;
			case 'telephonic':	$('#shipment').text('できtel');
							break;
			default:		$('#shipment').text('その他');
							break;
		}
		...
		if($('#dire_title .printtype_name').text()!='商品のみ'){
			...
			for(tab_title in mypage.temp.tabs){
				...
				if(print_type=='silk'){
					...
					switch(print_type){
						case 'inkjet':
							if(areasize_id=='0') size_select = '大（40×28）cm';
							else if(areasize_id=='1') size_select = '中（28×20）cm' ;
							else size_select = '小（15×15）cm' ;
							break;
						case 'trans':
						case 'digit':
							if(areasize_id=='0') size_select = '大（38×27）cm';
							else if(areasize_id=='1') size_select = '中（27×18）cm' ;
							else size_select = '小（10×10）cm' ;
							break;
						case 'digit':
						case 'cutting':
							if(areasize_id=='0') size_select = '大（40×30）cm';
							else if(areasize_id=='1') size_select = '中（30×10）cm' ;
							else size_select = '小（10×10）cm' ;
							break;
					}
				}
		...
		switch($(my).attr('name')){
			case 'cancel':
				...
				$.ajax({url: './php_libs/ordersinfo.php', type: 'POST',
					data: {'act':'update','mode':'acceptstatus','field1[]':field,'data1[]':data}, async: false,
	 				success: function(r){
	 					if(!r.match(/^\d+?$/)){
	 						alert('Error: p4971\n'+r);
	 						return;
	...
	*/
		...
		$.ajax({url:'./php_libs/ordersinfo.php', type:'POST', async:false,
			data:{'act':'update','mode':'sendmailcheck', 'field1[]':['fldname','check','orders_id'], 'data1[]':[myname,check,orders_id]}, success:function(r){
				if(r.trim()==''){
					alert('Error: p5566\n'+r);
					return;
				}
	...
	*/
		...
		if(act=='shipped'){
			$.post('./documents/shipmentmail.php', {'orders_id':data}, function(r){
				alert(r);
			...
			$.ajax({url:'./documents/sendmail.php', type:'POST', dataType:'json', async:false, data:{'doctype':act, 'json':1, 'data[]':data,'parm':parm}, 
				success:function(r){
					if(r instanceof Array){
						if(!parm){
							mypage.screenOverlay(true);
							if(r.length==0){
								...
								if(r[0].match(/^ERROR/)){
									...
									$('#mailer_wrapper').css({'top':offsetY+'px'}).fadeIn();
								}
							}
						}else{
	...
	// 確定注文の場合に読み取り専用にする
		if(mypage.prop.firmorder){
			...
			$('#noprint, #exchink_count').attr('disabled', true);											// プリントなし、イレギュラー
	...
	*/
		...
		if(mypage.prop.ordertype=='general'){
			...
			var list = {'act':'orderlist', 'ordertype':'general', 'isprint':isPrint, 'curdate':mypage.prop.firmorderdate, 'state':mypage.prop.firmorder};
			for(var key in sess){
							...
							$('#orderlist tbody tr').each( function(){
								...
								if($('#pp_wrapper span[title="item_'+item_id+'"]').length==0){
									...
									if( $('#pp_toggler_'+category_id).length==0 ){
										...
										$.ajax({url: './php_libs/dbinfo.php',type: 'POST',dataType: 'text',async: false,
											data: {'act':'ppbox', 'ppID':ppID, 'category_id':category_id, 'orders_id':info['orders_id'], 'mode':mode, 'ordertype':mypage.prop.ordertype},
							 				success: function(r){
							 					if(r.trim()!=""){
							 						var data = r.split('|');
													...
													html += '<input type="hidden" value="0" size="8" class="digit_price" />';
													html += '<input type="hidden" value="0" size="8" class="inkjet_price" />';
													html += '<input type="hidden" value="0" size="8" class="cutting_price" /></div>';
													html +=	'<p class="title">'+category_name+'<span title="item_'+item_id+'">'+item_name+'</span></p>';
													html += '</div>';
													$('#pp_wrapper').append(html).append(togglebody);
												}else{
			...
			var list = {'act':'orderlistext', 'curdate':mypage.prop.firmorderdate, 'state':mypage.prop.firmorder};
			for(var key in sess){
				list[key] = JSON.parse(sess.getItem(key));
			}
			$.ajax({url:'./php_libs/dbinfo.php',type: 'POST',dataType: 'text',async:false, data:list, 
				success:function(r){
					var tot_amount = 0;
					var tot_price = 0;
					var isNotBring = false;	// 持込ではない商品の有無
							...
							$('#orderlist tbody tr').each( function(){
								...
								if($('#pp_wrapper span[title="item_'+item_id+'"]').length==0){
									...
									if( $('#pp_toggler_'+category_id).length==0 ){
										var category_name = $(this).children('td:eq(2)').text();
										$.ajax({url: './php_libs/dbinfo.php',type: 'POST', dataType: 'text', async: false,
											data: {'act':'ppbox', 'ppID':ppID, 'orders_id':info['orders_id'], 'ordertype':mypage.prop.ordertype},
											success: function(r){
												if(r.trim()!=""){
													var data = r.split('|');
													var togglebody = '<div class="pp_toggle_body">'+data[0]+'</div>';
	...
	*/
		...
		$('#pp_wrapper .pp_toggler').each( function(){
			...
			tmp[cat] = {'id':ids, 'pp':pos, 'id_count':id_count};
		});
	...
	main: function(func){
		var btn = function(my){
			...
			$.ajax({url:'./php_libs/ordersinfo.php', type:'POST', dataType:'json', async:true, data:{'act':'search','mode':'accepting', 'field1[]':field, 'data1[]':data}, 
				success:function(r){
					if(r instanceof Array){
						if(r.length==0){
							mypage.screenOverlay(false);
							...
							*/
							...
							$('#result_searchtop tbody tr').each(function(i){
								if(i%4==0 || i%4==1) $(this).children('td').css({'background':'#f6f6f6'});
								if(i%4==0 || i%4==2) $(this).children('td').css({'border-top':'1px solid #d8d8d8'});
							});
		...
		*/
		var orderpage = function(){
			...
			var i = 0;
			var info = [];
			$.ajax({url:'./php_libs/ordersinfo.php', async:false, dataType:'json', data:{'act':'search','mode':'top', 'field1[]':['id'], 'data1[]':[arguments[0]]}, 
				success: function(r){
					if(r instanceof Array){
						info = r[0];
					}else{
			...
			$.ajax({url:'./php_libs/ordersinfo.php', type:'POST', dataType:'json', async:false, data:{'act':'search', 'mode':'bundlecount', 'field1[]':['orders_id','shipped'], 'data1[]':[info['orders_id'],1]}, 
				success:function(r){
					if(r instanceof Array){
						if(r.length>1){
							$('#bundle_status').show();
			...
			// 見積ボックス
			var ary1 = ['productfee','silkprintfee','colorprintfee','digitprintfee','inkjetprintfee','cuttingprintfee',
			...
			var ary2 = ['est_price','est_silk_printfee','est_color_printfee','est_digit_printfee','est_inkjet_printfee','est_cutting_printfee',
			...
			var media_rec = [];
			$.ajax({url:'./php_libs/ordersinfo.php', type:'POST', dataType:'text', async:false,
				data:{'act':'search','mode':'media', 'field1[]':["orders_id"], 'data1[]':[info['orders_id']]}, success:function(r){
					if(r=="") return;
					r = $.getDelimiter(r);
			...
			var discount_val = [];
			$.ajax({url:'./php_libs/ordersinfo.php', type:'POST', dataType:'text', async:false,
				data:{'act':'search','mode':'discount', 'field1[]':["orders_id"], 'data1[]':[info['orders_id']]}, success:function(r){
					if(r=="") return;
					r = $.getDelimiter(r);
			...
			*/
			...
			sess.clear();
			$.ajax({url:'./php_libs/ordersinfo.php', type:'POST',dataType:'json', async:false,
				data:{'act':'search','mode':'orderitem', 'field1[]':["orders_id"], 'data1[]':[info['orders_id']]}, success:function(r){
					if(r instanceof Array){
						var args = [];
						var master = 0;
						var hash = {};
						for(var i=0; i<r.length; i++){
							...
							if(typeof hash[r[i]['item_id']]=='undefined'){
								...
								hash[r[i]['item_id']] = {'name':r[i]['item_name'],'vol':parseInt(r[i]['amount'],10),'cost':item_cost,'fee':parseInt(r[i]['item_printfee'],10),'one':r[i]['item_printone']};
							}else{
								item_cost = r[i]['item_cost']*r[i]['amount'];
